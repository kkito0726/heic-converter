...
- ファイルごとにサイズを表示し、サーバーの上限(base64膨張込みで実効約48MiB/枚)超過を送信前に警告する
...
sequenceDiagram
    ...
    loop 各ファイル(最大3並列)
        ...
        Client->>Server: POST /heic.v1.ConvertService/Convert (JSON/base64)
...
| RPCクライアント | **@connectrpc/connect-web** | 既存のproto(`proto/heic/v1`)から生成した型安全クライアントでConnectプロトコル(JSON)を話す |
