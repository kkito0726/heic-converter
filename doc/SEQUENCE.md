...
(`internal/presentation/cli/root.go`の`run`)。

```mermaid
sequenceDiagram
    actor User as ユーザー
    participant Main as cmd/main.go
    participant Root as cli.New().Run (root.go)
    participant Interactive as interactive.go
    participant TUI as tui.go (bubbletea)
    participant Plain as runPlain (root.go)
...
```mermaid
sequenceDiagram
    participant Caller as 呼び出し元 (tui.go / root.go)
    participant Conv as usecase.Converter
