...
`.heic` / `.heif` 画像を jpg / png / webp / tiff / bmp / gif に変換する、対話型のCLIツールです。同じバイナリの `serve` サブコマンドでWebサーバーにもなり、ブラウザ用UI(`web/`)も同梱しています。
...
3. 出力先・上書き可否などを確認
4. プログレスバーを眺めながら変換完了を待つ(`q` または `Ctrl+C` でキャンセル)

フラグで指定済みの項目はスキップされるため、「形式はいつも同じでパスだけ対話で選ぶ」といった使い方もできます。

```sh
heic-converter ./photos -f jpg   # 形式は固定、残りのオプションだけ対話で確認
```
...
- **非TTY環境(パイプ・リダイレクト・CI)では自動でプレーンテキスト出力**になり、対話UIは起動しません。この場合はパスと `--format` の指定が必須です
- 既存の出力ファイルがあるとそのファイルは失敗として記録されます(上書きしたい場合は `--overwrite`)
- 1件でも失敗があると失敗一覧をレポートし、**終了コードは非ゼロ**になります(スクリプトから検知可能)
- 並列変換のワーカー数はCPUコア数に自動追従します
...
CLIが主役ですが、同じバイナリをWebサーバーとしても使えます。詳細な要件は [doc/connect-rpc-server/prd.md](doc/connect-rpc-server/prd.md) / [doc/frontend-web-ui/prd.md](doc/frontend-web-ui/prd.md) を参照してください。

### serve サブコマンド

単一エンドポイントで **gRPC / gRPC-Web / Connect (HTTP+JSON)** の3プロトコルを受け付けます。スキーマは [proto/heic/v1/convert.proto](proto/heic/v1/convert.proto) です。
...
- サーバーリフレクション対応のため `grpcurl` からprotoファイルなしで叩けます。ヘルスチェック(gRPC Health Checking Protocol)も実装済みです
...
```sh
heic-converter serve   # 1. APIサーバーを起動
cd web && npm install && npm run dev   # 2. フロント開発サーバーを起動
```
...
起動後 http://localhost:3000 を開くとWeb UIが使えます。`web`(nginx)が静的ファイルを配信し、RPCパスを `api` コンテナへリバースプロキシします(同一オリジンのためCORS不要。`api` はホストへポート公開しません)。

本番公開のためのインフラ構成(Cloudflare Pages + Cloud Run、ほぼ0円運用)は [doc/deployment/infrastructure-design.md](doc/deployment/infrastructure-design.md) にまとまっています。
...
クリーンアーキテクチャを採用しています。domain層がデコーダ/エンコーダ/ストレージのinterface(port)を定義し、infra層がそれを実装するため、ライブラリの差し替えが容易です。入力層(presentation)はusecaseのみに依存しており、実際にCLIとWebサーバー(connect-rpc)が同じ変換コアを共有しています。
